...
class HGPSLModel(torch.nn.Module):
    ...
    def forward(self, graph, n_feat, explain_mode=False):
        final_readout = None
        e_feat = None
        ...
        for i in range(self.num_layers):
            graph, n_feat, e_feat, readout, x_score, perm = self.convpool_layers[i](
                graph, n_feat, e_feat
            )
            if explain_mode:
                scores_list.append(x_score)
        ...
        if explain_mode:
            return F.log_softmax(n_feat, dim=-1), scores_list, perms_list
