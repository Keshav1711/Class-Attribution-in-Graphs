...
def parse_args():
    ...
    parser.add_argument(
        "--epochs", type=int, default=10, help="max number of training epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument("--visualize", action="store_true", help="visualize node importance")
...
def compute_importance(batch_graph, scores_list, perms_list, use_pool_list):
    importance = torch.zeros(batch_graph.num_nodes(), device=scores_list[0].device)
    current_selected = torch.arange(batch_graph.num_nodes(), device=scores_list[0].device)
    for i in range(len(scores_list)):
        x_score = scores_list[i]
        if use_pool_list[i]:
            perm = perms_list[i]
            mask = torch.ones(len(current_selected), dtype=torch.bool, device=x_score.device)
            mask[perm] = False
            discarded = current_selected[mask]
            importance[discarded] = x_score[mask]
            current_selected = current_selected[perm]
        else:
            importance[current_selected] = x_score
    return importance

def visualize_graph(graph, importance, title="Node Importance", output_dir="graph_visualizations"):
    nx_graph = dgl.to_networkx(graph.cpu())
    nx_graph.remove_edges_from(nx.selfloop_edges(nx_graph))
    node_colors = importance.cpu().numpy()
    plt.figure(figsize=(10, 10))
    nx.draw(nx_graph, node_color=node_colors, cmap=plt.cm.Blues, with_labels=True)
    plt.colorbar(plt.cm.ScalarMappable(cmap=plt.cm.Blues), label="Node Importance")
    plt.title(title)
    ...
    if args.visualize:
        ...
        with torch.no_grad():
            for batch_graphs, batch_labels in test_loader:
                ...
                out, scores_list, perms_list = model(batch_graphs, batch_graphs.ndata["feat"], explain_mode=True)
                use_pool_list = [True] * (args.conv_layers - 1) + [False]
                importance = compute_importance(batch_graphs, scores_list, perms_list, use_pool_list)
                graphs = dgl.unbatch(batch_graphs)
                importance_offsets = torch.cumsum(torch.tensor([0] + [g.num_nodes() for g in graphs[:-1]]), dim=0)
                for i, g in enumerate(graphs):
                    start = importance_offsets[i].item()
                    end = start + g.num_nodes()
                    g_importance = importance[start:end]
                    visualize_graph(g, g_importance, title=f"Graph {i} Node Importance")
